...
function ReportsPage() {
    ...
    // Expiry Date = Join Date + Plan Duration (months from Plan column)
    const toMemberShape = React.useCallback((lead) => {
        ...
        // Parse months from plan label
        const label = lead.membership?.planLabel || lead.interest || lead.plan || '';
        ...
        if (monthMatch) {
            ...
            durationMonths = parseInt(yearMatch[1]) * 12;
        }
        if (!durationMonths) {
            durationMonths = lead.membership?.durationMonths || lead.membership?.duration || lead.membership?.planDuration || null;
        }

        // Always calculate expiry date as join date + plan months
        let expiryDate = null;
        if (durationMonths && startDate) {
            ...
            expiryDate = calculatedEnd.toISOString();
        } else {
            expiryDate = null;
        }
            ...
            <section className="mt-8 rounded-3xl bg-white p-6 ring-1 ring-gray-100">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={handleExportAllMembers} className="px-3 py-1.5 bg-gray-600 text-white rounded-full text-sm font-medium hover:bg-gray-700">Export All</button>
                        <button onClick={handleExportPageMembers} className="px-3 py-1.5 bg-gray-500 text-white rounded-full text-sm font-medium hover:bg-gray-600">Export Page</button>
                    </div>
                </div>
